...
export default function ChatPage() {
  ...
  // Career Coach specific state
  const [useCareerCoach, setUseCareerCoach] = useState(false);
  const [tavilyApiKey, setTavilyApiKey] = useState('');
  const [cohereApiKey, setCohereApiKey] = useState('');
  const [langchainApiKey, setLangchainApiKey] = useState('');
  const [useReranking, setUseReranking] = useState(false);
  const [toolCalls, setToolCalls] = useState<any[]>([]);
  const [contextUsed, setContextUsed] = useState<any[]>([]);
  ...
  const systemPromptTemplates = [
    ...
    { 
      ...
      prompt: 'You are an expert AI career coach. Help users with job market insights, career advice, skill development, salary information, and professional growth in the AI/tech industry.' 
    },
    { 
      name: 'Code Expert', 
  ...
  // On mount, load API keys from sessionStorage and listen for changes
  useEffect(() => {
    ...
    setUseCareerCoach(sessionStorage.getItem('USE_CAREER_COACH') === 'true');
  ...
  const handleSend = async (event?: FormEvent) => {
    ...
    try {
      if (useCareerCoach) {
        // Use Career Coach endpoint
        const body = {
          user_message: userMessage,
          api_key: apiKey,
          tavily_api_key: tavilyApiKey || undefined,
          cohere_api_key: cohereApiKey || undefined,
          langchain_api_key: langchainApiKey || undefined,
          model,
          use_reranking: useReranking
        };
        
        const response = await axios.post(`${getApiUrl()}/career-coach`, body);
  ...
  return (
    <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Career Coach Mode Indicator */}
      {useCareerCoach && (
        <Box sx={{ p: 1, background: 'linear-gradient(135deg, #e8f5e8 60%, #c8e6c9 100%)', borderTop: '1px solid #4caf50', borderBottom: '1px solid #ececec' }}>
          <Typography variant="caption" sx={{ color: '#2e7d32', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            AI Career Coach Mode Active
            {toolCalls.length > 0 && (
              <Chip 
                label={`${toolCalls.length} tools used`} 
                size="small" 
                sx={{ ml: 1, background: '#4caf50', color: 'white', fontSize: '0.7em' }} 
              />
            )}
          </Typography>
          {contextUsed.length > 0 && (
            <Typography variant="caption" sx={{ color: '#2e7d32', display: 'block', mt: 0.5 }}>
              {contextUsed.length} relevant documents found
            </Typography>
          )}
        </Box>
      )}
